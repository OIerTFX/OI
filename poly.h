...
namespace pol{
	...
	struct poly
	{
		...
		void conv(const poly& f,const poly& g){
			int tmp=n;n=1<<int(ceil(log2(f.n+g.n+1)));
			static int *F=new int[n],*G=new int[n];
			memcpy(F,f.a,sizeof(int)*f.n);
			memcpy(G,g.a,sizeof(int)*g.n);
			poly(F,n).NTT();poly(G,n).NTT();
			for(int i=0;i<n;i++) a[i]=(ll)F[i]*G[i]%MOD;
			INTT();delete[] F;delete[] G;n=tmp;
		}
		...
		void ln(const poly& f){
			...
			poly(g,n).diff(f);poly(g,n).NTT();
			inv(f);NTT();
			for(int i=0;i<n;i++) a[i]=(ll)a[i]*g[i]%MOD;
			INTT();
		}
		void ln(){ln(*this);}
